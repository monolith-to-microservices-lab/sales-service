...
VALID_AFTER = {"id": 501, "user_id": 1, "item_name": "Widget", "quantity": 2, "created_at": "2026-01-01T00:00:00Z"}


class TestSuccessCases:
    def test_create_event(self):
        envelope = DebeziumSaleEnvelope.model_validate({"before": None, "after": VALID_AFTER, "op": "c"})
        assert envelope.op == "c"
        assert envelope.after.id == 501
        assert envelope.after.user_id == 1

    def test_read_snapshot_event(self):
        envelope = DebeziumSaleEnvelope.model_validate({"before": None, "after": VALID_AFTER, "op": "r"})
        assert envelope.op == "r"

    def test_update_event_has_both_before_and_after(self):
        before = {**VALID_AFTER, "quantity": 1}
        envelope = DebeziumSaleEnvelope.model_validate({"before": before, "after": VALID_AFTER, "op": "u"})
        assert envelope.before.quantity == 1
        assert envelope.after.quantity == 2

    def test_delete_event_uses_before_after_is_none(self):
        envelope = DebeziumSaleEnvelope.model_validate({"before": VALID_AFTER, "after": None, "op": "d"})
    ...
    def test_unknown_extra_fields_are_ignored(self):
        envelope = DebeziumSaleEnvelope.model_validate(
            {"before": None, "after": VALID_AFTER, "op": "c", "transaction": None,
...
class TestFailureCases:
    ...
    def test_payload_missing_id_raises(self):
        with pytest.raises(ValidationError):
            SaleCdcPayload.model_validate(
                {"user_id": 1, "item_name": "Widget", "quantity": 1, "created_at": "2026-01-01T00:00:00Z"}
            )

    def test_payload_missing_user_id_raises(self):
        with pytest.raises(ValidationError):
            SaleCdcPayload.model_validate(
                {"id": 1, "item_name": "Widget", "quantity": 1, "created_at": "2026-01-01T00:00:00Z"}
            )

    def test_payload_id_wrong_type_raises(self):
        with pytest.raises(ValidationError):
            SaleCdcPayload.model_validate(
                {"id": "not-a-number", "user_id": 1, "item_name": "W", "quantity": 1,
    ...
    def test_payload_quantity_wrong_type_raises(self):
        with pytest.raises(ValidationError):
            SaleCdcPayload.model_validate(
                {"id": 1, "user_id": 1, "item_name": "W", "quantity": "many",
                 "created_at": "2026-01-01T00:00:00Z"}
            )
    ...
    def test_payload_invalid_timestamp_raises(self):
        with pytest.raises(ValidationError):
            SaleCdcPayload.model_validate(
                {"id": 1, "user_id": 1, "item_name": "W", "quantity": 1, "created_at": "not-a-timestamp"}
            )
