"""Standalone CDC consumer: `legacy.public.sales` (Kafka/Debezium) -> this
...
only keeps the Sales Service's copy in sync. Structurally identical to
user-service's app/cdc/consumer.py - see that file for the reasoning behind
each design choice; only the entity (Sale vs. User) and its fields differ.
...
successfully (see `apply_sale_event`). If the database write fails, the
...
user_id handling: the Sales Service's `Sale` model has no foreign key to a
local `users` table (see app/models.py) - user_id is stored as a plain
integer, exactly as it arrives in the event. There is nothing to validate
locally, so a sale event referencing a user_id not yet present anywhere is
still applied normally: no synchronous call to user-service, no blocking, no
ordering dependency between the users and sales topics.
"""
...
from . import metrics
from .apply import apply_sale_event
from .events import DebeziumSaleEnvelope

logger = logging.getLogger("sales.cdc")
settings = get_settings()

try:
    from opentelemetry import trace

    _tracer: trace.Tracer | None = trace.get_tracer("sales.cdc")
...
class SalesCdcConsumer:
    def __init__(
        ...
        self._topic = topic or settings.kafka_sales_topic
    ...
    def process_message(self, message) -> None:
        ...
        envelope = DebeziumSaleEnvelope.model_validate(json.loads(raw_value))
        payload = envelope.after or envelope.before
        entity_id = payload.id if payload else None
        user_id = payload.user_id if payload else None
        log_extra = {**log_base, "op": envelope.op, "sale_id": entity_id, "user_id": user_id}

        with _start_span("sales.cdc.process") as span:
            ...
            if entity_id is not None:
                span.set_attribute("cdc.sale_id", entity_id)
            if user_id is not None:
                span.set_attribute("cdc.user_id", user_id)

            session = self._session_factory()
            db_start = time.perf_counter()
            try:
                with _start_span("sales.cdc.db.apply"):
                    apply_sale_event(session, envelope)
    ...
    def _commit_offset(self, message) -> None:
        with _start_span("sales.cdc.kafka.commit"):
            try:
