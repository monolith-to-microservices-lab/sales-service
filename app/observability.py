"""Prometheus metrics + /metrics endpoint for sales-service.

Kept separate from RequestContextMiddleware (correlation id) so that
middleware's tested behavior is untouched. OpenTelemetry tracing is
intentionally NOT added here yet - there is no CDC consumer in this service
to trace end-to-end; this only prepares the metrics side so the architecture
is ready if/when a sales consumer is built (see cdc-infrastructure's
legacy.public.sales topic, already flowing with no consumer attached).
"""
